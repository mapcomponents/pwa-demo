...
const SearchContextProvider: React.FC<SearchContextProviderProps> = ({
  ...
  const [searchIndexInstance, setSearchIndexInstance] = useState<
    any | undefined
  >(undefined);
  ...
  useEffect(() => {
    if (searchTerm && searchIndexInstance) {
      const results = searchIndexInstance.search(searchTerm.toString(), {
        fields,
      });
      const maxResults = 10;
  ...
  useEffect(() => {
    if (selectedResult && mapHook.map) {
      if (
        ...
        "COORDINATES" in selectedResult
      ) {
        const createdFeature = createGeoJSONFeature(
          selectedResult.COORDINATES as Coordinates
        );
